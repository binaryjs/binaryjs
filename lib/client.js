...
function BinaryClient(socket, options) {
  ...
  if(typeof socket === 'string') {
    this._nextId = 0;
    this._socket = new WebSocket(socket, this._options.ws || {});
  } else {
      ...
      switch(data[0]) {
        ...
        case 7:
          ...
          if(binaryStream) {
            ...
            self.emit('warn', new Error('Received `notification` message for unknown stream: ' + streamId));
          }
          break;
        case 8:
          var message = data[1];          
          ...
          break;
        default:
          self.emit('warn', new Error('Unrecognized message type received: ' + data[0]));
      }
    //});
  });
}
